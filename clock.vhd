...
architecture rtl of tutorial_led_blink is
  ...
  -- Formula is: (8 MHz / 100 Hz * 50% duty cycle)
  -- So for 100 Hz: 8,000,000 / 100 * 0.5 = 400,000
  constant c_CNT_100HZ : natural := 40000;
  constant c_CNT_50HZ  : natural := 80000;
  constant c_CNT_10HZ  : natural := 400000;
  constant c_CNT_1HZ   : natural := 4000000;
...
begin
  ...
  w_LED_SELECT <= r_TOGGLE_100HZ when i_SW = "00" else
                  r_TOGGLE_50HZ  when i_SW = "10" else
                  r_TOGGLE_10HZ  when i_SW = "01" else
                  r_TOGGLE_1HZ;
 
   
  -- Only allow o_led_drive to drive when i_enable is high (and gate).
  o_LD <= w_LED_SELECT; --and i_enable;
